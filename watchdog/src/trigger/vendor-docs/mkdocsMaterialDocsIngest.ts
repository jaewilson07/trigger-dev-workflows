...
/**
 * New sync — Material for MkDocs' docs (setup, reference for every extension/plugin, customization, guides), from squidfunk/mkdocs-material's `docs/` subpath, minus blog/changelog/insiders (see the registry entry). Member of the MkDocs docs set — see mkdocsDocsIngest.ts.
 *
...
type MkdocsMaterialDocsIngestPayload = {
...
export const mkdocsMaterialDocsIngest = schedules.task({
  id: "mkdocs-material-docs-ingest",
  cron: {
  ...
  maxDuration: 1800,
  run: async (_payload: MkdocsMaterialDocsIngestPayload, { ctx }) => {
    ...
    try {
      return await withVendorDocsFailureReporting(SOURCE.id, ghPat, ctx.run.id, async () => {
        logger.info("starting mkdocs-material-docs-ingest", {
          subfolder: SOURCE.subfolder,
          collectionName: SOURCE.collectionName,
        });
        const result = await runVendorDocsGitMirrorTask(SOURCE, { ghPat, dcToken });
        logger.info("completed mkdocs-material-docs-ingest", {
      ...
      logger.error("failed mkdocs-material-docs-ingest", {
