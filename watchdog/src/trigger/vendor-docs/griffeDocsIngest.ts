...
/**
 * New sync — Griffe's docs (the static analyzer mkdocstrings-python is built on: loading packages, docstring parsing, extensions, API-diff checks), from mkdocstrings/griffe's `docs/` subpath. Member of the MkDocs docs set — see mkdocsDocsIngest.ts.
 *
...
const SOURCE = VENDOR_DOCS_GIT_MIRROR_SOURCES.find((s) => s.id === "griffe-docs")!;

type GriffeDocsIngestPayload = {
...
export const griffeDocsIngest = schedules.task({
  id: "griffe-docs-ingest",
  cron: {
  ...
  maxDuration: 1800,
  run: async (_payload: GriffeDocsIngestPayload, { ctx }) => {
    ...
    try {
      return await withVendorDocsFailureReporting(SOURCE.id, ghPat, ctx.run.id, async () => {
        logger.info("starting griffe-docs-ingest", {
          subfolder: SOURCE.subfolder,
          collectionName: SOURCE.collectionName,
        });
        const result = await runVendorDocsGitMirrorTask(SOURCE, { ghPat, dcToken });
        logger.info("completed griffe-docs-ingest", {
      ...
      logger.error("failed griffe-docs-ingest", {
