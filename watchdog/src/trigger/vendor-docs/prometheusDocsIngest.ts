...
/**
 * New sync — the project-wide half of prometheus.io/docs (introduction,
 * concepts, instrumenting, practices, guides, specs), from prometheus/docs'
 * `docs/` subpath. The server reference half (configuration, PromQL,
 * storage) is prometheusServerDocsIngest.ts — see `vendorDocsIngest.ts`'s
 * registry doc comment on this source for why it is two sources. Part of
 * the Grafana OSS monitoring-stack docs set — see grafanaDocsIngest.ts.
 *
...
const SOURCE = VENDOR_DOCS_GIT_MIRROR_SOURCES.find((s) => s.id === "prometheus-docs")!;

type PrometheusDocsIngestPayload = {
...
export const prometheusDocsIngest = schedules.task({
  id: "prometheus-docs-ingest",
  cron: {
  ...
  // Small corpus (~70 files); budget kept identical to its siblings.
  maxDuration: 1800,
  run: async (_payload: PrometheusDocsIngestPayload, { ctx }) => {
    ...
    try {
      return await withVendorDocsFailureReporting(SOURCE.id, ghPat, ctx.run.id, async () => {
        logger.info("starting prometheus-docs-ingest", {
          subfolder: SOURCE.subfolder,
          collectionName: SOURCE.collectionName,
        });
        const result = await runVendorDocsGitMirrorTask(SOURCE, { ghPat, dcToken });
        logger.info("completed prometheus-docs-ingest", {
      ...
      logger.error("failed prometheus-docs-ingest", {
