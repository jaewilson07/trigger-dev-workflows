 ...
 * ## Shared, not copied
 *
 * This lived as three byte-identical copies under `executive-assistant/lib/`,
 * `storm-research/lib/` and `watchdog/src/lib/`, on the grounds that each
 * project deploys as its own artifact and so needed a real shared package
 * rather than a relative import reaching outside the project root. That
 * package is `@datacrew/trigger-shared`, and this file now lives in it.
 * (`lib/google-docs.ts` is still triplicated on the old grounds.)
 */
