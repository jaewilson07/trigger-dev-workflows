...
/**
 * New sync — mkdocstrings' docs (usage, handler configuration, cross-references, recipes, troubleshooting), from mkdocstrings/mkdocstrings' `docs/` subpath — the plugin that renders API reference pages from docstrings. Member of the MkDocs docs set — see mkdocsDocsIngest.ts.
 *
...
const SOURCE = VENDOR_DOCS_GIT_MIRROR_SOURCES.find((s) => s.id === "mkdocstrings-docs")!;

type MkdocstringsDocsIngestPayload = {
...
export const mkdocstringsDocsIngest = schedules.task({
  id: "mkdocstrings-docs-ingest",
  cron: {
  ...
  maxDuration: 1800,
  run: async (_payload: MkdocstringsDocsIngestPayload, { ctx }) => {
    ...
    try {
      return await withVendorDocsFailureReporting(SOURCE.id, ghPat, ctx.run.id, async () => {
        logger.info("starting mkdocstrings-docs-ingest", {
          subfolder: SOURCE.subfolder,
          collectionName: SOURCE.collectionName,
        });
        const result = await runVendorDocsGitMirrorTask(SOURCE, { ghPat, dcToken });
        logger.info("completed mkdocstrings-docs-ingest", {
      ...
      logger.error("failed mkdocstrings-docs-ingest", {
