...
/**
 * New sync — Grafana Alloy's docs (grafana.com/docs/alloy: the collector
 * that replaced Grafana Agent — component reference, configuration syntax,
 * collecting logs/metrics/traces). Part of the Grafana OSS monitoring-stack
 * docs set — see grafanaDocsIngest.ts.
 *
 * Mirrors grafana/alloy's `docs/sources` subpath into
 * `vendor-docs-sync/alloy-docs/`, then ingests that subfolder. See
 * `vendorDocsIngest.ts`'s registry doc comment on this source.
 *
...
export const alloyDocsIngest = schedules.task({
  id: "alloy-docs-ingest",
  cron: {
  ...
  // Sized against comfyuiDocsIngest.ts's comparable budget; alloy-docs'
  // corpus is ~400 files / ~3.9MB.
  maxDuration: 1800,
  run: async (_payload: AlloyDocsIngestPayload, { ctx }) => {
    ...
    try {
      ...
      logger.error("failed alloy-docs-ingest", {
