 ...
 * `infra-bonker/.agents/runbooks/ingest-trigger-dev-skills/ingest-trigger-dev-skills.sh`
 * (`0 9 * * *`) — jaewilson07/trigger-dev-workflows#128. Same shape and same
 * problems as the `ingest-letta-docs.sh` script this project's
 * `lettaDocsIngest.ts` also replaces (copy-pasted script, no Trigger.dev
 * equivalent, no failure alerting beyond a local log file).
 *
 * Mirrors `triggerdotdev/skills` (whole repo — matching the bonker script's
 * scope) into `vendor-docs-sync/trigger-dev-skills/`, then ingests that
 * subfolder with trigger-dev-skills' existing, explicit `collection_id`. See
...
const SOURCE = VENDOR_DOCS_GIT_MIRROR_SOURCES.find((s) => s.id === "trigger-dev-skills")!;

type TriggerDevSkillsIngestPayload = {
...
export const triggerDevSkillsIngest = schedules.task({
  id: "trigger-dev-skills-ingest",
  cron: {
  ...
  maxDuration: 1800,
  run: async (_payload: TriggerDevSkillsIngestPayload, { ctx }) => {
    ...
    try {
      return await withVendorDocsFailureReporting(SOURCE.id, ghPat, ctx.run.id, async () => {
        logger.info("starting trigger-dev-skills-ingest", {
          subfolder: SOURCE.subfolder,
          collectionId: SOURCE.collectionId,
        });
        const result = await runVendorDocsGitMirrorTask(SOURCE, { ghPat, dcToken });
        logger.info("completed trigger-dev-skills-ingest", {
      ...
      logger.error("failed trigger-dev-skills-ingest", {
